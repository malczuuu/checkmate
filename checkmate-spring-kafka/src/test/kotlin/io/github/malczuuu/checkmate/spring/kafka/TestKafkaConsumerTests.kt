...
import io.github.malczuuu.checkmate.container.KafkaAwareTest
...
@ContainerTest
@SpringBootTest(classes = [TestKafkaApplication::class, KafkaTestAutoConfiguration::class])
internal class TestKafkaConsumerTests : KafkaAwareTest {

  @Autowired private lateinit var kafkaOperations: KafkaOperations<String, String>

  @TestListener(TestKafkaApplication.TOPIC_IN) private lateinit var consumerIn: TestKafkaConsumer

  @TestListener(TestKafkaApplication.TOPIC_OUT) private lateinit var consumerOut: TestKafkaConsumer
  ...
  fun givenPublishedMessages_whenClear_thenSubsequentPollReturnsEmpty() {
    ...
    await().atMost(Duration.ofSeconds(15)).pollInterval(Duration.ofMillis(200)).untilAsserted {
      assertThat(consumerIn.poll(Duration.ofMillis(500))).isNotEmpty()
    }
  ...
  fun givenPublishedMessageWithHeader_whenPoll_thenHeaderIsAccessible() {
    val record = ProducerRecord(TestKafkaApplication.TOPIC_IN, "key-3", "value-3")
    ...
    await().atMost(Duration.ofSeconds(15)).pollInterval(Duration.ofMillis(200)).untilAsserted {
      ...
      assertThat(records).anySatisfy { r ->
        assertThat(r.headers().lastHeader("event_type"))
            .isNotNull()
            .satisfies({ h: Header ->
              assertThat(String(h.value(), StandardCharsets.UTF_8)).isEqualTo("TestEvent")
            })
      }
    }
  }
  ...
  fun givenMessageOnTopicIn_whenForwarded_thenAppearsOnTopicOutOnly() {
    ...
    await().atMost(Duration.ofSeconds(15)).pollInterval(Duration.ofMillis(200)).untilAsserted {
      ...
      assertThat(consumerOut.poll(Duration.ofMillis(500))).anySatisfy { r ->
        assertThat(r.value()).isEqualTo("forwarded-msg-x")
      }
    }
  }
  ...
  fun givenNoTopicConsumerFields_whenCreateContextCustomizer_thenReturnsNull() {
    ...
    assertThat(factory.createContextCustomizer(NoListenerClass::class.java, emptyList())).isNull()
  }

  @Test
  fun givenTopicConsumerField_whenCreateContextCustomizer_thenReturnsCustomizer() {
    val factory = TestKafkaConsumerContextCustomizerFactory()
    assertThat(factory.createContextCustomizer(TestKafkaConsumerTests::class.java, emptyList()))
        .isNotNull()
  }
